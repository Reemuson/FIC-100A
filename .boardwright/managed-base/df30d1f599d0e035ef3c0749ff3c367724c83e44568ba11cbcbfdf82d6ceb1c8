...
def dispatch_preview(plan: PreviewPlan, root: Path) -> None:
    ...
    completed = subprocess.run(
        list(_preview_dispatch_command(plan)),
        cwd=root,
        text=True,
        capture_output=True,
        check=False,
...
def preview_manual_fallback(plan: PreviewPlan) -> str:
    return "\n".join(
        [
            ...
            "",
            "Manual fallback:",
            f"GitHub Actions: {plan.workflow} -> Run workflow",
            f"Ref: {plan.branch}",
            "Inputs:",
            ...
            " ".join(_preview_dispatch_command(plan)),
        ]
    )


def _preview_dispatch_command(plan: PreviewPlan) -> tuple[str, ...]:
    command = [
        plan.gh_command,
        "workflow",
        "run",
        ...
        raise BoardwrightError(
            "\n".join(
                [
                    ...
                    "",
                    "Manual fallback:",
                    f"GitHub Actions: {config.preview_workflow}",
